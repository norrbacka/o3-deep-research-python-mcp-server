...
def save_research_summary(
    ...
    sanitized_query = "".join(c for c in query if c.isalnum() or c in " _-")[:30].strip()
...
async def server_lifespan(mcp: FastMCP) -> AsyncIterator[None]:
    ...
    try:
        ...
        print(f"DeepResearchTool definitions: {tools_config}", file=sys.stderr)
        print(f"MODEL_DEPLOYMENT_NAME: {os.environ['MODEL_DEPLOYMENT_NAME']}", file=sys.stderr)
        print(f"DEEP_RESEARCH_MODEL_DEPLOYMENT_NAME: {os.environ['DEEP_RESEARCH_MODEL_DEPLOYMENT_NAME']}", file=sys.stderr)
        ...
        agent_id = agent.id
        # Use stderr for logging during development
    ...
    try:
        ...
        while run.status in ("queued", "in_progress") and poll_count < max_polls:
            ...
            await context.report_progress(
                progress=poll_count,
                total=max_polls,
                message=f"Run status: {run.status} (poll {poll_count}/{max_polls})"
            )
        ...
        if response_text and ("?" in response_text or "Would you like" in response_text):
            ...
            while run.status in ("queued", "in_progress") and poll_count < max_polls:
                ...
                await context.report_progress(
                    progress=poll_count,
                    total=max_polls,
                    message=f"Deep research in progress... (poll {poll_count}/{max_polls})"
                )
        ...
        # Save research summary and get file URI
        filepath_uri = save_research_summary(agent_response, query)
        await context.info(f"Research summary saved to: {filepath_uri}")
        
        return filepath_uri
        
    except Exception as e:
        await context.error(f"Error during deep research: {str(e)}")
        raise
